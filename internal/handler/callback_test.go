...
func TestHandleCallback_UserMessage(t *testing.T) {
	...
	h := NewCallbackHandler(agent, "", false, 0)
...
func TestHandleHealth(t *testing.T) {
	h := NewCallbackHandler(nil, "", false, 0)
	mux := http.NewServeMux()
	h.RegisterRoutes(mux, "/callback")
...
func TestHandleCallback_SignatureRequired(t *testing.T) {
	...
	h := NewCallbackHandler(agent, "my-secret", true, 0)
	mux := http.NewServeMux()
	h.RegisterRoutes(mux, "/callback")
...
func TestHandleCallback_AuthDisabledWithSecret(t *testing.T) {
	agent := &fakeAgent{reply: "收到"}
	h := NewCallbackHandler(agent, "my-secret", false, 0)
	mux := http.NewServeMux()
	h.RegisterRoutes(mux, "/callback")
...
func TestHandleCallback_BodyTooLarge(t *testing.T) {
	agent := &fakeAgent{reply: "收到"}
	h := NewCallbackHandler(agent, "", false, 10)
	mux := http.NewServeMux()
	h.RegisterRoutes(mux, "/callback")

	body := `{"eventType":"user_message","message":{"id":1,"content":"this body is definitely larger than ten bytes"}}`
	req := httptest.NewRequest(http.MethodPost, "/callback", strings.NewReader(body))
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
